...
//        ){
	...
	/*___ email to friend and feed subcription ___*/
    ...
    function get_next_prev_posts() {
		...
		// modification next previous link add thumbail ' . get_the_post_thumbnail( $prev_post->ID, array(100,70) ) .'
        $buffy = '';
        ...
        if (!empty($next_post) or !empty($prev_post)) {
            ...
            if (!empty($prev_post)) {
                ...
                $buffy .= '' . get_the_post_thumbnail( $prev_post->ID, array(218,150) ) .'<a href="' . esc_url(get_permalink($prev_post->ID)) . '">' . get_the_title( $prev_post->ID ) . '</a>';
                $buffy .= '</div>';
                $buffy .= '</div>';
            } else {
                $buffy .= '<div class="td-block-span6 td-post-prev-post">';
            ...
            if (!empty($next_post)) {
                ...
                $buffy .= '' . get_the_post_thumbnail( $next_post->ID, array(218,150) ) .'<a href="' . esc_url(get_permalink($next_post->ID)) . '">' . get_the_title( $next_post->ID ) . '</a>';
                $buffy .= '</div>';
                $buffy .= '</div>';
            }
